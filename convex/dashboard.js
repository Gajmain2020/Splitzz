...
export const getTotalSpent = query({
  handler: async (ctx) => {
    ...
    userExpense.forEach((e) => {
      const userSplit = e.splits.find((split) => split.userId === user._id);
...
export const getMonthlySpending = query({
  handler: async (ctx) => {
    ...
    for (let i = 0; i < 12; i++) {
      ...
      monthlyTotal[monthDate.getTime()] = 0;
    }
    ...
    userExpense.forEach((expense) => {
      ...
      if (userSplit) {
        monthlyTotal[monthStart] =
          (monthlyTotal[monthStart] || 0) + userSplit.amount;
      }
    });
